...
4. Enter the following parameters:
    ...
    12. `Enable Partial Transcripts` - Enable partial transcripts to receive low latency evolving transcriptions for each conversation turn. 
    13. `Enable Content Redaction for Transcripts` - Enable content redaction from Amazon Transcribe transcription output. **NOTE:** Content redaction is only available when using the English language (en-US). This parameter is ignored when not using the English language
    14. `Language for Transcription` - Language code to be used for Amazon Transcribe
