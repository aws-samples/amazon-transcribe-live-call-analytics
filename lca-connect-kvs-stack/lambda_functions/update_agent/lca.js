...
const writeUpdateAgentToKds = async function writeUpdateAgentToKds(kinesisClient, callId, agentId) {
  console.log('Write Update Agent Event to KDS');
  const putObj = {
    CallId: callId,
    AgentId: agentId,
  ...
  console.log('Sending UPDATE_AGENT event on KDS: ', JSON.stringify(putObj));
  const putCmd = new PutRecordCommand(putParams);
  try {
    await kinesisClient.send(putCmd);
  } catch (error) {
    console.error('Error writing UPDATE_AGENT', error);
  }
};
...
exports.writeUpdateAgentToKds = writeUpdateAgentToKds;
