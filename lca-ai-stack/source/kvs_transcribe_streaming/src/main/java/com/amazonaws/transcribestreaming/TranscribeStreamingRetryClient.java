...
/**
 * Build a client wrapper around the Amazon Transcribe client to retry on an
...
public class TranscribeStreamingRetryClient implements AutoCloseable {
    ...
    private static final boolean IS_CONTENT_REDACTION_ENABLED = Boolean
            .parseBoolean(System.getenv().getOrDefault("IS_CONTENT_REDACTION_ENABLED", "false"));
    private static final String TRANSCRIBE_LANGUAGE_CODE = System.getenv().getOrDefault("TRANSCRIBE_LANGUAGE_CODE",
            "en-US");
    private static final String CONTENT_REDACTION_TYPE = System.getenv().getOrDefault("CONTENT_REDACTION_TYPE", "PII");
    private static final String PII_ENTITY_TYPES = System.getenv().getOrDefault("PII_ENTITY_TYPES", "ALL");
    private static final String CUSTOM_VOCABULARY_NAME = System.getenv().getOrDefault("CUSTOM_VOCABULARY_NAME", "");
     ...
     */
    public TranscribeStreamingRetryClient(AwsCredentialsProvider creds, String endpoint, Regions region,
            MetricsUtil metricsUtil) throws URISyntaxException {
        this(TranscribeStreamingAsyncClient.builder().credentialsProvider(creds).endpointOverride(new URI(endpoint))
                .region(Region.of(region.getName())).build(), metricsUtil);
    }
    ...
    private StartStreamTranscriptionRequest rebuildRequestWithSession(StartStreamTranscriptionRequest request) {
        logger.debug(String.format(
                "Setting client with language: [%s], redaction type: [%s], content redaction enabled: [%s], pii entity types: [%s] and custom vocabulary: [%s]",
                TRANSCRIBE_LANGUAGE_CODE, CONTENT_REDACTION_TYPE, IS_CONTENT_REDACTION_ENABLED, PII_ENTITY_TYPES, CUSTOM_VOCABULARY_NAME));
        StartStreamTranscriptionRequest.Builder builder = StartStreamTranscriptionRequest.builder()
                .languageCode(TRANSCRIBE_LANGUAGE_CODE).mediaEncoding(request.mediaEncoding())
                .mediaSampleRateHertz(request.mediaSampleRateHertz()).sessionId(UUID.randomUUID().toString());

        if (IS_CONTENT_REDACTION_ENABLED) {
            builder.contentRedactionType(CONTENT_REDACTION_TYPE);
            if(!PII_ENTITY_TYPES.isEmpty()){
                builder.piiEntityTypes(PII_ENTITY_TYPES);
            }
        }
        if (!CUSTOM_VOCABULARY_NAME.isEmpty()) {
            builder.vocabularyName(CUSTOM_VOCABULARY_NAME);
        }
    ...
    private StartStreamTranscriptionResponseHandler getResponseHandler(
        ...
        final StartStreamTranscriptionResponseHandler build = StartStreamTranscriptionResponseHandler.builder()
                .onResponse(r -> {
                    ...
                    // We swallow any exception occurred while processing the TranscriptEvent and
                    // continue transcribing
                    // Transcribe errors will however cause the future to complete exceptionally and
                    // we'll retry (if applicable)
                    catch (Exception e) {
                    }
                }).build();
        return build;
