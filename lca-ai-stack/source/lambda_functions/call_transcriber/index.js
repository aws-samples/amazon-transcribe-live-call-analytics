...
const { DynamoDBClient, BatchExecuteStatementCommand, QueryCommand, PutItemCommand } = require("@aws-sdk/client-dynamodb");
...
const getStreamsFromDynamo = async function(callId, agentArn, callerArn) {
  ...
  let command = new QueryCommand(dynamoParams);
...
const splitAudio = function(audioBuffer, maxSize) {
    ...
    while (audioBuffer.length > 0) {
        ...
        //console.log('i:' + i);
        result.push(audioBuffer.slice(0, i));
...
const go = async function (callId, lambdaCount, agentStreamArn, callerStreamArn, sessionId, lastAgentFragment, lastCallerFragment) { 
  ...
  let tempRecordingFilename = callId + "-" + lambdaCount + ".raw";
  ...
  let combinedStreamBlock = new BlockStream(4); //TODO: Calculate this size based on 250ms 'chunks' 
  ...
  var transcribePromise = readTranscripts(tsStream, callId, callerStreamArn, sessionId);

  var returnVals = await Promise.all([
    callerWorker,
    agentWorker
  ]);
...
const handler = async function (event, context) {
  ...
  if(event.detail.streamingStatus === "CONTINUE") {
    ...
    var streamResults = await getStreamsFromDynamo(event.detail.callId, agentStreamArn, callerStreamArn);
    ...
    if(streamResults.agentStreamArn == undefined || streamResults.callerStreamArn == undefined) {
      ...
      result = await go(event.detail.callId,0,streamResults.agentStreamArn, streamResults.callerStreamArn, undefined, undefined, undefined);
    }
  }
  ...
  if(result) 
  {
    if(timeToStop) {
      ...
      let invokeCmd = new InvokeCommand({
      ...
      writeStatusToDynamo('STEREO', 'END_TRANSCRIPT', event.detail.callId, event.detail.streamArn, (event.detail.transcribeSessionId ? event.detail.transcribeSessionId : '')); 
    }
